export async function generateTip(
  context: string
): Promise<{ tip: string; title: string; explanation: string }> {
  ...
  if (!apiKey) {
    ...
    return {
      ...
      explanation:
        "The `console.table()` method displays tabular data as a table. This function takes one mandatory argument data, which must be an array or an object, and one additional optional argument columns.",
    }
  }
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        explanation:
          "Proper indentation and comments make your code easier to read and maintain for yourself and others.",
      }
    }
  } catch (error) {
    ...
    return {
      ...
      explanation:
        "It seems we had trouble generating a new tip. But here's a default one: Try to use try-catch blocks for async operations!",
    }
  }
}
